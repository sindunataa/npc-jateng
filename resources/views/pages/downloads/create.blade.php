                        ...
                        <div class="card-body">
                            <form action="{{ route('downloads.store')}}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="">
                                    ...
                                    @error('title')
                                            <label id="title-error" class="error mt-2 text-danger"
                                                for="title-error">{{ $message }}</label>
                                    @enderror
                                    <div class="form-group">
                                        <label for="path" class="form-label">Upload Files :</label>
                                        <input type="file" name="path" class="dropify" data-height="250" data-width="200"/>
                                    </div>
                                    <div class="form-group">
                                        <label for="published_at" class="form-label">Diterbitkan Pada :</label>
                                        <input type="date" name="published_at" class="form-control" id="published_at" placeholder="published_at">
                                    </div>
                                    @error('published_at')
                                            <label id="published_at-error" class="error mt-2 text-danger"
                                                for="published_at-error">{{ $message }}</label>
                                    @enderror
                                </div>
